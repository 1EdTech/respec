...
(function () {
  ...
  function run$11(config) {
    config.l10n = l10n$x[lang$3] || l10n$x.en;
  }

  var l10n$y = /*#__PURE__*/Object.freeze({
    __proto__: null,
    name: name$1a,
    l10n: l10n$x,
    lang: lang$3,
...
for more information.`;
  ...
  marked$1.use = function(...args) {
    ...
    args.forEach((pack) => {
      ...
      if (pack.renderer) {
        const renderer = marked$1.defaults.renderer || new Renderer$2();
  ...
  function joinFactory(type, style = "long") {
    const formatter = new Intl.ListFormat(lang$3, { style, type });
    /**
     * @template T
     * @param {string[]} items
...
>
  ...
  const modules = [
    ...
    Promise.resolve().then(function () { return l10n$y; }),
...
>
  ...
  function personValidator(prop) {
    ...
    return function validatePerson(person, index) {
      ...
      if (!person.name) {
        ...
        showError(msg, name$_, { hint: seePersonHint });
        return false;
      }
  ...
  var headersTmpl = (conf, options) => {
    ...
    <details open="">
      <summary>${l10n$w.more_details_about_this_doc}</summary>
      <dl>
  ...
  function renderFeedback(conf) {
    ...
    if (conf.wgPublicList) {
      ...
      const subject =
        conf.subjectPrefix ?? `[${conf.shortName}] ${l10n$w.your_topic_here}`;
    ...
    >`;
      ...
      const subjectLine =
        conf.subjectPrefix ||
        html`[${conf.shortName}] <em>${l10n$w.message_topic}</em>`;
      const emailSubject = html`${l10n$w.with_subject_line}${" "}
      <kbd>${subjectLine}</kbd>`;
    ...
    return html`<dt>${l10n$w.feedback}</dt>
    ${definitions}`;
  }
  ...
  function renderHistory(conf) {
    ...
    if (conf.historyURI) {
      const dd = html`<dd>
      <a href="${conf.historyURI}">${l10n$w.publication_history}</a>
    </dd>`;
      ddElements.push(dd);
    }
    if (conf.github) {
      const dd = html`
      <dd>
        <a href="${conf.github.commitHistoryURL}">${l10n$w.commit_history}</a>
      </dd>
    `;
      ddElements.push(dd);
    }

    return html`<dt>${l10n$w.history}</dt>
    ${ddElements}`;
  }
  ...
  const W3CNotes = ["FPWD-NOTE", "WG-NOTE"];
  ...
  async function run$T(conf) {
    ...
    if ((conf.isCGBG || !conf.isNoTrack || conf.isTagFinding) && !sotd.id) {
      ...
      showError(msg, name$Y, { elements: [sotd], hint });
    }
...
`;
  ...
  Object.keys(additions).forEach(key => {
    if (!l10n$x[key]) l10n$x[key] = {};
    Object.assign(l10n$x[key], additions[key]);
  });
  ...
  function computeExport(dfn) {
    switch (true) {
      ...
      case dfn.matches(".export.no-export"): {
        ...
        showError(msg, name$I, { elements: [dfn], hint });
        break;
      }
  ...
  function validateDefinition(text, type, dfn) {
    ...
    if (entry.requiresFor && !dfn.dataset.dfnFor) {
      ...
      showError(msg, name$I, { hint, elements: [dfn] });
    }

    if (entry.validator) {
      entry.validator(text, type, dfn, name$I);
    }
  }
    ...
    return tests
      .map(test => {
        try {
          ...
          showWarning(msg, name$j, { elements: [elem] });
        }
      })
      .filter(href => href);
  }
